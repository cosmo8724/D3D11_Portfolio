...
CHellheim_Island::CHellheim_Island(const CHellheim_Island & rhs)
	: CGameObject(rhs)
{
}

HRESULT CHellheim_Island::Initialize_Prototype()
{
...
HRESULT CHellheim_Island::Initialize(const wstring & wstrPrototypeTag, void * pArg)
{
...
HRESULT CHellheim_Island::Render()
{
...
HRESULT CHellheim_Island::Render_Reflect()
{
...
pair<_bool, _float3> CHellheim_Island::Picking_Mesh()
{
	...
	vPos = XMVectorSetX(vPos, XMVectorGetX(vPos) - 400.f);
	vPos = XMVectorSetZ(vPos, XMVectorGetZ(vPos) - 600.f);
...
HRESULT CHellheim_Island::SetUp_Component()
{
...
HRESULT CHellheim_Island::SetUp_ShaderResource()
{
...
HRESULT CHellheim_Island::SetUp_ShaderResource_Reflect()
{
	NULL_CHECK_RETURN(m_pShaderCom, E_FAIL);
...
CHellheim_Island * CHellheim_Island::Create(DEVICE pDevice, DEVICE_CONTEXT pContext)
{
	CHellheim_Island*		pInstance = new CHellheim_Island(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Createe : CHellheim_Island");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CHellheim_Island::Clone(const wstring & wstrPrototypeTag, void * pArg)
{
	CHellheim_Island*		pInstance = new CHellheim_Island(*this);

	if (FAILED(pInstance->Initialize(wstrPrototypeTag, pArg)))
	{
		MSG_BOX("Failed to Clone : CHellheim_Island");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CHellheim_Island::Free()
{
	__super::Free();
