#include "stdafx.h"
#include "..\Public\Shop_Island.h"
#include "GameInstance.h"

CShop_Island::CShop_Island(DEVICE pDevice, DEVICE_CONTEXT pContext)
	: CGameObject(pDevice, pContext)
{
}

CShop_Island::CShop_Island(const CShop_Island & rhs)
	: CGameObject(rhs)
{
}

HRESULT CShop_Island::Initialize_Prototype()
{
...
HRESULT CShop_Island::Initialize(const wstring & wstrPrototypeTag, void * pArg)
{
	...
	m_pTransformCom->Set_State(CTransform::STATE_TRANS, XMVectorSet(575.f, 0.196f, 328.f, 1.f));

	return S_OK;
}

void CShop_Island::Tick(_double dTimeDelta)
{
	__super::Tick(dTimeDelta);
}

void CShop_Island::Late_Tick(_double dTimeDelta)
{
...
pair<_bool, _float3> CShop_Island::Picking_Mesh()
{
...
HRESULT CShop_Island::SetUp_Component()
{
...
HRESULT CShop_Island::SetUp_ShaderResource()
{
...
CShop_Island * CShop_Island::Create(DEVICE pDevice, DEVICE_CONTEXT pContext)
{
	CShop_Island*		pInstance = new CShop_Island(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Createe : CShop_Island");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CShop_Island::Clone(const wstring & wstrPrototypeTag, void * pArg)
{
	CShop_Island*		pInstance = new CShop_Island(*this);

	if (FAILED(pInstance->Initialize(wstrPrototypeTag, pArg)))
	{
		MSG_BOX("Failed to Clone : CShop_Island");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CShop_Island::Free()
{
	__super::Free();
