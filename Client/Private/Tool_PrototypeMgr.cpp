		...
		if (bSave)
		{
			...
			if (ImGuiFileDialog::Instance()->Display("Select Folder"))
			{
				if (ImGuiFileDialog::Instance()->IsOk())
				{
					...
					CGameUtility::ctwc(m_pLevelName[iSaveLevel], wszLevelName);
					...
					for (auto& Pair : m_mapProtoComponenets[iSaveLevel])
					{
						...
						string	strComponentTag = CGameUtility::WstrToStr(Pair.first);
						string strFilePath = CGameUtility::WstrToStr(Pair.second->Get_FilePath());
		...
		if (bSave)
		{
			...
			if (ImGuiFileDialog::Instance()->Display("Select Save Folder"))
			{
				if (ImGuiFileDialog::Instance()->IsOk())
				{
					...
					for (auto& Pair : *m_mapProtoObjects)
					{
						CCustomGameObject*	pObject = dynamic_cast<CCustomGameObject*>(Pair.second);
						// 일단은 CustomGameObject만 저장 해보기.
						if (pObject != nullptr)
						{
							char	szPrototypeGameObjectTag[MAX_PATH] = "";
							CGameUtility::wctc(Pair.first.c_str(), szPrototypeGameObjectTag);
							...
							for (auto PairCom : vecComponentInfo)
							{
								...
								CGameUtility::wctc(PairCom.second.c_str(), szPrototypeComponentTag);
...
COMPONENTTYPE CTool_PrototypeMgr::CheckComponentType(CComponent * pComponent)
{
	NULL_CHECK_RETURN(pComponent, COMPONENTTYPE_END);
