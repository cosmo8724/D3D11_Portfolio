...
void CSound::Set_SoundDesc(SOUND_DESC& SoundDesc)
{
	...
	_vector	vStart = m_SoundDesc.pStartTransform->Get_State(CTransform::STATE_TRANS);
	_vector	vEnd = m_SoundDesc.pTargetTransform->Get_State(CTransform::STATE_TRANS);
...
CSound* CSound::Create(FMOD_SOUND * pSound)
{
	...
	if (FAILED(pInstance->Initialize(pSound)))
	{
